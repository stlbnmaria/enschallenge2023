...
def write_mocov_train(data_path=Path("../storage")):
    """
    ...
    # store csv
    output = np.vstack(
        (y_train[1:], patients_train[1:], samples_train[1:], centers_train[1:], coords[1:])
    ).T
    np.savez_compressed(
...
def write_mocov_test(data_path=Path("../storage")):
    ...
    features_dir = data_path / "test_input" / "moco_features"
    ...
    # store csv
    output = np.vstack((patients_train[1:], samples_train[1:], centers_train[1:], coords[1:])).T
    np.savez_compressed(
